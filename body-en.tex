...
Contours can be understood as the shape or format of an
object. \nota{por que vc se recusa a dizer ``contours are'', e dis
  ``can be understood''? Até onde eu sei, contornos são formas.} In
Music, they can be associated to elements like pitch, density, rhythm,
and represent a parameter in function of another, such as pitch in
function of time. Contours can be easily recognized from a graphic
representation by professionals and laymen alike
...
from the lowest to the highest \cite{morris93:directions}\nota{que tal
  em vez de ``contour is defined'' dizer, ``technically, a contour is
  an ordered set of numbered elements'', ou algo assim? Acho que fica
  mais simples}. In contours\nota{em vez de ``In contours'', que tal
  algo como ``when representing something as a contour'' ou algo
  assim?}, absolute values of its elements are ignored, and only the
...
sets, contours help giving coherence to a musical piece. They
areq structural devices that can be combined through operations
...
combinations \nota{precisa do ``and combinations''?} in musical
composition are scarce, despite the possible coherence that contours
can give and the \nota{and the what of operations? operações por si só
  não dizem nada, é bom dizer por que são interessantes, ou até que
  são interessantes, ou então tirar essa segunda parte da frase}
